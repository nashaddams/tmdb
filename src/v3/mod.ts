...
export class TmdbApi {
   ...
   *
   * @param {{ credit_id: string; language?: string; }} opts
   * @see https://developer.themoviedb.org/v3/reference/credit-details
   */
  async creditDetails(
    opts: { credit_id: string; language?: string },
  ): Promise<
    {
      credit_type: string;
      department: string;
  ...
  > {
    ...
    return await (await fetch(
      `${this.#base_url}/3/credit/${opts.credit_id}?api_key=${this.#api_key}${queryParams}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
